...
| Three cloud resource names are hard-coded, and one contradicts Terraform | `deploy.sh:L23`, `:L31` and `:L35` hard-code them, and `infrastructure/terraform/main.tf:L51` declares the only tracked bucket as `word-documents-${var.project_id}`, which does not match `deploy.sh:L23` |

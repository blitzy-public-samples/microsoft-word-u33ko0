...
| **20. Coverage and verbosity are measured separately, which is how conflict C4 dissolves.** Completeness is proved by the 257-row matrix below. Length is held down by the 150-to-400-line band on the module READMEs and by the short-cell rule in every table. | Read the instruction to be exhaustive as licence for long prose, or read Rule 3's brevity position as licence to leave constructs undocumented. | Rule 3 rejects thoroughness that destroys readability, and the requirements demand exhaustive coverage. The two collide only while one measure stands for both. Counting coverage in matrix rows and readability in sentence and paragraph length satisfies both standards at once, so no trade-off was made. | Neither number alone proves quality. A 257-row matrix of dull cells would pass the coverage measure and fail the reader, which is why `prose-validation.md` carries the second half of the proof. A reader who equates completeness with volume also reads the short cells as thin. |
| **21. The dependency register is split into a risk table and a complete inventory.** [troubleshooting.md](troubleshooting.md#the-dated-dependency-and-advisory-register) carries nine risk rows first, then all 47 distributions. | One table of 47 rows carrying every advisory history, or a risk table alone with no inventory behind it. | Forty-seven advisory histories in one table bury the nine rows that change what a reader does. A risk table alone cannot show that nothing was omitted. Two tables let absence from the first mean "queried, nothing found" rather than "unlisted", and the inventory states which of the two applies per row. | A reader who stops at the risk table sees nine distributions and may assume the other 38 went unexamined. Two tables also drift apart when a distribution is added to one and not the other. |
...
| **24. [onboarding.md](onboarding.md) pins every version it names.** Both version-manager tags, the seventeen backend distributions and the `passlib` and `bcrypt` pair carry an exact version, and the pins live in prose because no manifest may be created. | Name the distributions and the tags without versions and let each reader's resolver choose, which is what an unpinned install command does. | An unpinned command is not a reproducible clean-machine path, because a resolver picks a different set on a later date. One such pair, passlib 1.7.4 with bcrypt 5.0.0, raises on every call. A pinned command in prose is the only reproducible form available while the minimal change clause forbids a committed manifest. | Pins in prose age without warning, and no tool in this repository checks them. A reader may also read a pin as a project requirement rather than as a diagnostic aid, so each pinned command has to say which it is. |
...
| decision-log.md (this file) | Twenty-four decisions, eleven deviations and the bidirectional matrix | All 257 matrix rows | None |

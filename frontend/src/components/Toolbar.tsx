 ...
 *   L12, `AppDispatch` at L13 and a default `store` at L15, and nothing else.
 ...
 *   and `clearRecentDocuments` at L43-L50, and the reducer as its default at L52.
 ...
 *
 * @remarks
 * Side effects: each of the six style buttons dispatches a Redux action, through L15 for the
 * inline buttons at L31-L33 and through L20 for the block buttons at L36-L38. The two insert
 * buttons at L41 and L42 reach `console.log` at L25 and change no state.
 *
 ...
 *
 * A second type error sits on those same two lines. Both helpers return an `EditorState`, per the
 * `EditorState.push` returns at `formatting.ts:L13` and `:L26`. L14 and L19 bind that
 * `EditorState` to `updatedContent`, and L15 and L20 then dispatch it as a `content` value.
 *
 ...
 * heading: `ToolbarProps` at L487-L491 declares `onBoldClick`, `onItalicClick` and
 * `onUnderlineClick`, and L493 types the component as `React.FC<ToolbarProps>`. The committed
 * component at L10 declares no props and dispatches to the store instead.
 *

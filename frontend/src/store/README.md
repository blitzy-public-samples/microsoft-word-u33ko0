...
bindings, while `documentSlice.ts:L93` and `userSlice.ts:L76` export their reducer as the default. Those two lines carry the
...
for local user and profile state. The second tree is not an authentication record. `userSlice.ts:L42` sets `isAuthenticated`
true at `L44` on any `setUser` dispatch. The slice holds no token, checks no signature and calls no server, so nothing here
...
| `documentSlice.ts` | Module | `documentSlice.ts:L14-L98` | Declares the `document` slice with four state fields at L18-L23 and six reducers. |
| `setCurrentDocument` | Reducer and action | `documentSlice.ts:L44` | Writes `state.currentDocument` from a `Document` payload. The only action a consumer imports. |
| `addRecentDocument` | Reducer and action | `documentSlice.ts:L54` | Prepends the payload and keeps four earlier entries, so the list holds five at most. |
| `setLoading` | Reducer and action | `documentSlice.ts:L58` | Writes `state.isLoading` from a boolean payload. |
| `setError` | Reducer and action | `documentSlice.ts:L62` | Declares `PayloadAction<string \| null>`, so `setError(null)` clears the document error. |
| `clearCurrentDocument` | Reducer and action | `documentSlice.ts:L66` | Sets `state.currentDocument` to null. Takes no `action` parameter. |
| `clearRecentDocuments` | Reducer and action | `documentSlice.ts:L70` | Empties `state.recentDocuments`. Takes no `action` parameter. |
| Document action creators | Named export | `documentSlice.ts:L84-L91` | Destructures all six creators from `documentSlice.actions`. |
| Document reducer | Default export | `documentSlice.ts:L93` | `documentSlice.reducer`, exported under no other name. |
| `userSlice.ts` | Module | `userSlice.ts:L12-L83` | Declares the `user` slice with four state fields at L16-L21 and four reducers. |
| `setUser` | Reducer and action | `userSlice.ts:L42` | Writes four fields at L43-L46: the user, `isAuthenticated` true, `isLoading` false and a null error. |
| `clearUser` | Reducer and action | `userSlice.ts:L49` | Resets the same four fields at L50-L53. Takes no `action` parameter. |
| `setLoading` | Reducer and action | `userSlice.ts:L56` | Writes `state.isLoading` from a boolean payload. |
| `setError` | Reducer and action | `userSlice.ts:L60` | Declares `PayloadAction<string>`, narrower than `documentSlice.ts:L62`, so no payload clears the user error. Also sets `isLoading` false at L62. |
| User action creators | Named export | `userSlice.ts:L75` | Destructures `setUser`, `clearUser`, `setLoading` and `setError` on one line. |
| User reducer | Default export | `userSlice.ts:L76` | `userSlice.reducer`, exported under no other name. |
...
| --- | --- | --- | --- |
| `index.ts` | `documentReducer` from `./documentSlice` | `index.ts:L15` | Fails. `documentSlice.ts:L93` exports the reducer as the default and declares no `documentReducer`. |
| `index.ts` | `userReducer` from `./userSlice` | `index.ts:L16` | Fails. `userSlice.ts:L76` exports the reducer as the default and declares no `userReducer`. |
...
| --- | --- | --- |
| Recent-document list length | Five entries at most | `documentSlice.ts:L55` |
| Document slice initial state | `currentDocument` null, `recentDocuments` empty, `isLoading` false, `error` null | `documentSlice.ts:L26-L31`, fields at L27-L30 |
| User slice initial state | `currentUser` null, `isAuthenticated` false, `isLoading` false, `error` null | `userSlice.ts:L24-L29`, fields at L25-L28 |
| Slice names | `'document'` and `'user'` | `documentSlice.ts:L40`, `userSlice.ts:L38` |
...
graph TD
    ...
    DSLICE["documentSlice.ts<br/>reducer is the<br/>default export, L93"]
    USLICE["userSlice.ts<br/>reducer is the<br/>default export, L76"]
...
Slice per domain. Each slice declares its state shape, initial state and reducers in one module. `documentSlice.ts:L39-L74`
covers documents and `userSlice.ts:L37-L65` covers the user, and neither slice reads the other's state.

Draft mutation through Immer. Every reducer in both slices assigns to a field on `state` and returns nothing, as
`documentSlice.ts:L45` and `userSlice.ts:L43-L46` show. Redux Toolkit hands the reducer a draft object and derives the next
...
- **The store never constructs.** `index.ts:L15` and `:L16` import `documentReducer` and `userReducer`, while
  `documentSlice.ts:L93` and `userSlice.ts:L76` export their reducer as the default. Both identifiers appear nowhere else in
...
- **`updateDocument` and `selectCurrentDocument` are absent from `documentSlice.ts`.** `components/DocumentCanvas.tsx:L17`
  ...
  `:L46` and `:L57`. The slice exports six creators at `documentSlice.ts:L84-L91` and neither name is among them. A separate
  `updateDocument` is an application programming interface (API) client function at `services/api.ts:L94`, imported at
  `pages/Editor.tsx:L16`; the two names are unrelated.
- **`userSlice.ts` exports neither `selectCurrentUser` nor `updateUser`.** `userSlice.ts:L75` exports `setUser`,
...
- **`addRecentDocument` caps the recent list at five, and the literal in the code reads `4`.** `documentSlice.ts:L55`
...
- **The two slices treat a cleared error differently.** `documentSlice.ts:L62` declares `PayloadAction<string | null>`, so
  `setError(null)` clears the document error. `userSlice.ts:L60` declares `PayloadAction<string>`, so no payload clears the
  user error and only `clearUser` at `userSlice.ts:L49` resets it. `userSlice.ts:L62` also sets `isLoading` to `false`,
  which `documentSlice.ts:L62-L64` does not.
...
- **Both slices carry an end-of-file assistance marker.** `documentSlice.ts:L95-L98` and `userSlice.ts:L78-L83` each sit
  after the export statements. The user-slice marker names the missing `updateUser` action at `:L81` and the missing
  selectors at `:L82`.
...
Dispatching a document action uses the six creators exported at `documentSlice.ts:L84-L91`, with payload fields from
...
A sixth `addRecentDocument` dispatch drops the oldest entry, because `documentSlice.ts:L55` retains four. `setError(null)`
clears the document error, because `documentSlice.ts:L62` accepts null.

Dispatching a user action uses the four creators exported on `userSlice.ts:L75`, with payload fields from `UserSchema` at
...
`setUser` also sets `isAuthenticated` to true and clears the error at `userSlice.ts:L44-L46`, and it does so for any payload
that satisfies the `User` type. No token accompanies the dispatch and no server confirms it, so the flag records a local
claim rather than a verified session. `clearUser` at `userSlice.ts:L49` is the one path that clears the user error.
...
`documentSlice.ts:L93` and `userSlice.ts:L76`. `documentSlice.ts` lacks the `updateDocument` action that

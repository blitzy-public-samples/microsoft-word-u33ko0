 ...
 * reach it. `backend/app/main.py:L50` mounts the documents router before `:L51` mounts the
 * users router, and `GET /{document_id}` at `backend/app/api/documents.py:L22` compiles to the
 ...
 * `document_id` bound to the literal string `me`. `PUT /me` at `backend/app/api/users.py:L12`
 * is shadowed the same way, by `PUT /{document_id}` at `backend/app/api/documents.py:L30`.
 ...
 *
 * No committed line reads the key back. `frontend/src/services/api.ts:L14-L23` builds its bearer
 * header from the Redux store rather than from `localStorage`, so the stored value never reaches
 * an outgoing request.
 *
 * The request contract does not match the server's token route. L7 sends an HTTP POST with a
 * JavaScript object body carrying `email` and `password`, which Axios serializes as JSON under
 * `Content-Type: application/json`. `backend/app/api/auth.py:L29` declares
 ...
 * `frontend/src/services/api.ts:L16`, reads `store.getState().auth.token` instead, and
 * `frontend/src/store/index.ts:L6-L9` registers no `auth` reducer key for it to read.
 * Correcting the field name at L8 fixes the stored value and leaves the flow disconnected,
 ...
 * the page origin can read this key through the same interface L9 writes with. The value also
 ...
 * library. L7 calls the unconfigured default Axios export with no `timeout`, no `signal` and no
 * `AbortController`, and no retry, backoff, jitter, circuit breaker or fallback surrounds it.
 * L12 discards the original error, so a caller cannot read a status code to decide whether a
 ...
 * `POST /auth/logout`, so L18 rejects in practice, L19 never runs, and L21 writes to the
 ...
 *
 * L21 logs the whole error object rather than a message. An Axios error carries `config`,
 ...
 *
 * Resilience is absent here for the same reasons recorded on `login`. L18 calls the
 * unconfigured default Axios export with no `timeout`, no `signal` and no
 * `AbortController`, and no retry,
 * backoff, jitter, circuit breaker or fallback surrounds it. L21 keeps the original error in
 ...
 * user object.
 *
 * L27 sends no credential of any kind. The request skips the bearer-token interceptor that
 * `frontend/src/services/api.ts:L14-L23` installs, and no line here reads the `accessToken`
 ...
 *
 * Resilience is absent here for the same reasons recorded on `login`. L27 calls the
 * unconfigured default Axios export with no `timeout`, no `signal` and no
 * `AbortController`, and no retry,
 * backoff, jitter, circuit breaker or fallback surrounds it. L30 discards the original error.
 *
 * @example
 ...
 * // backend/app/api/users.py:L8, and the header above records why the assembled route order

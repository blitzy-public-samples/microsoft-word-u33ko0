...
- That first request answers 500 instead. The handler raises `TypeError` at `:L91`, because it calls `get_document(document_id)` with one argument against the two the signature at `../../../backend/app/services/document_service.py:L78` requires. `POST /documents` at `api.ts:L82` matches the same single-segment shape for which no router declares `POST`, so Starlette answers 405 rather than 404. `PUT /documents/${documentId}` at `api.ts:L95` carries two segments that no route declares, so it answers 404.

...
for all thirteen required distributions sits in [../README.md](../README.md).
...
| --- | --- | --- |
| `google-cloud-firestore` | Unestablished | `from google.cloud.firestore import Client` at `firestore.py:L36`, constructed at `firestore.py:L42`. |
| `google-auth` | Unestablished | `from google.auth import default` at `firestore.py:L37`, called at `firestore.py:L41`. |
...
`GOOGLE_CLOUD_PROJECT` unset and `firestore.py:L42` can then build the client with `project=None`. The inner `Config`
...
step that runs. The client at `firestore.py:L42` is never constructed, because the settings import
at `firestore.py:L38` raises first. A dashed edge marks a relationship that cannot resolve at all.
...
graph TD
    ...
    CLIENT["firestore.py:L42 db = Client(...)<br/>CONDITIONAL: never built, L38 raises first"]
    ...
    DSVC -.->|"L116, L170, L236, L275<br/>call site resolves, client absent"| CLIENT
...
Four patterns appear across the two modules, and one expected pattern does not. `firestore.py:L42` builds a
module-level singleton, so one client exists per process and every importer shares it. The four helpers wrap that
client thinly and synchronously, each resolving a document reference at `firestore.py:L66`, `L91`, `L109` or `L126`
...
- **Managed provisioning: none.** `infrastructure/terraform/main.tf` declares one provider, four
  ...
  Nothing in the repository provisions the Firestore database that `firestore.py:L42` connects to,
...
- **Application use of that server: none.** The local PostgreSQL service is provisioned and
  unused. `sql.py` has no importer anywhere in `backend/`, `Base` at `sql.py:L19` is never
  subclassed, `get_db` at `sql.py:L21` has no consumer, no ORM model is declared, and no migration
  tool is configured. Every request-path persistence call in the committed code goes to Firestore
...
- **`main.py` expects three things from this folder, and one of them does not exist.**
  ...
  `firestore.py:L42`. No dependency manifest pins `google-cloud-firestore`, so whether either
...
`firestore.py:L44` and `firestore.py:L72`. `update_document` and `delete_document` take the same two-line shape, so neither
...
The call above cannot run. `firestore.py:L38` imports the absent `settings` singleton, so the import statement raises

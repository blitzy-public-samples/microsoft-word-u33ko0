...
`config.py:L61`. `security.py` declares a password-hashing context at `security.py:L22`, a bearer-token scheme at
`security.py:L23`, and four functions covering JSON Web Token (JWT) issuance, password verification, password hashing,
...
| `Settings.Config` | Nested Pydantic configuration class | `config.py:L50` | Sets `env_file = ".env"` at `L58` and `env_file_encoding = "utf-8"` at `L59`. The named `.env` file is not committed. |
| `get_settings` | Factory function | `config.py:L61` | Returns a fresh `Settings` instance at `L74`. Performs no caching. |
...
bottom of every import chain: eight modules request a `settings` object from it, and `security.py:L20` requests the
...
**No `.env` file is committed.** `config.py:L58` points `env_file` at `.env`, and no such file exists in the
...
Two flows pass through this package. Configuration flows outward: a caller invokes `get_settings()` at `config.py:L61`,
and a fresh `Settings` instance returns at `config.py:L74`. Pydantic reads the process environment and the absent
...
sequenceDiagram
    ...
    GCU->>GCU: get_settings(), L123
    GCU->>GCU: jwt.decode, L123
    Note over GCU: FastAPI resolves the dependency, and oauth2_scheme<br/>extracts the header at security.py:L23<br/>get_settings() resolves at config.py:L61<br/>jose.jwt.decode reads SECRET_KEY and ALGORITHM at L123<br/>sub claim read at L124<br/>401 at L126 if sub is None, 401 at L128 on jwt.JWTError
    GCU--xUS: UserService(), L130
    Note over GCU,US: BROKEN EDGE, drawn dashed with a cross:<br/>UserService is undefined, so L130 raises NameError,<br/>await get_user_by_id at L131 never runs,<br/>and the 401 at L133 is never reached
```
...
factory, `get_settings()` at `config.py:L61`, rather than an exported instance.
...
`NameError: name 'Optional' is not defined` at `security.py:L25`, before any function in the module becomes callable.
Each example shows the declared contract, taken from the signature at its cited line.

`create_access_token` at `security.py:L25` takes a payload dictionary and an optional lifetime, and returns the encoded
token as `str`.
...
# Default lifetime, taken from settings.ACCESS_TOKEN_EXPIRE_MINUTES at security.py:L52
token = create_access_token({"sub": "user-123"})

# Explicit lifetime, which skips the settings read at security.py:L52
short_lived = create_access_token({"sub": "user-123"}, timedelta(minutes=5))
```

The call above cannot execute: the import raises `NameError` at `security.py:L25`. Both forms also require `SECRET_KEY`

...
| `DocumentCreate` | Pydantic model | `document.py:L30` | Create payload. The body is `pass` at L37, so the class inherits all three `DocumentBase` fields and adds none. |
| `DocumentUpdate` | Pydantic model | `document.py:L39` | Patch payload. Extends `BaseModel` and declares `title` at L48 and `content` at L49, both `Optional[str] = None`. |
| `Document` | Pydantic model | `document.py:L51` | Read response. Extends `DocumentBase` and adds `id: str` at L62, `created_at: datetime` at L63 and `updated_at: datetime` at L64. |
| `DocumentVersion` | Pydantic model | `document.py:L66` | Version record. Extends `BaseModel` with `id` at L80, `document_id` at L81, `content` at L82, `created_at` at L83 and `user_id` at L84. |
...
Nine `app.schema.*` import statements reach this package from seven modules, and eight resolve. The importers sit at `app/api/auth.py:L20`,
...
module persists a user, `api/auth.py:L21` imports the `UserService` that would, and that module does not exist.
...
`updated_at` at `document.py:L64`, and `username` at `user.py:L27` plus `full_name` at
...
| `app/api/auth.py` | `:L20` | Yes | `User`, `UserCreate` |
...
fails validation because the assembled mapping carries no `created_at` and no `updated_at`, both of which `document.py:L63-L64` declares as
required.
...
sits at `user.py:L17`, `:L30`, `:L40` and `:L56`, and the document set at `document.py:L16`, `:L30`, `:L39` and `:L51`.
...
Optional-field patch semantics. `DocumentUpdate` at `document.py:L39-L49` and `UserUpdate` at `user.py:L40-L54` declare
...
The family pattern breaks on both patch payloads. `DocumentUpdate` at `document.py:L39` and `UserUpdate` at
...
| Pydantic version contract | `user_id` | `document.py:L84` |
...
- **Three router sites read a field the read model never declares.** `Document` inherits `owner_id` and declares no
  `user_id`, yet `app/api/documents.py:L92`, `:L121` and `:L147` each read `.user_id` on a `Document`.
- **`created_at` and `updated_at` are required and no service writes either.** `document.py:L63-L64` declares both as
...
- **`DocumentUpdate` shares no field definition with the create path.** A field added to `DocumentBase` at
  `document.py:L16-L28` reaches `DocumentCreate` and `Document` and never reaches `DocumentUpdate` at `:L39`.
- **`DocumentCreate` adds nothing.** The body is `pass` at `document.py:L37`, so the create payload inherits the
...
- **`DocumentVersion` has no importer.** `document.py:L66` defines the model, and a search for the name across every
...
That block cannot succeed, because `document.py:L63-L64` requires `created_at` and `updated_at` while the assembled

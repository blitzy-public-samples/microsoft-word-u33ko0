...
| `PROJECT_ID` | READ-BUT-NEVER-DECLARED | First read at `services/collaboration_service.py:L120` | Pub/Sub topic and subscription paths, also `:L121`, `:L209` and `:L245` |
...
| `DOCUMENT_BUCKET_NAME` | READ-BUT-NEVER-DECLARED | Read once, at `tasks/background_tasks.py:L278` | The retention sweep `cleanup_expired_documents`, declared at `tasks/background_tasks.py:L152`. The statistics task at `:L287` does not read it. |
...
a misconfiguration. Each is unexploitable while import fails, and each becomes live the moment it is repaired.
...
- **Two instructions in the root `README.md` do not work against this package.** `L42` directs a reader to
  ...
  application object lives at `backend/app/main.py`. The root README is reference material here and receives no edit.

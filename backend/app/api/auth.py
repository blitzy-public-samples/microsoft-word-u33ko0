...
`app/main.py:L16` imports the name `auth_router` from this module. L12 defines
`router` instead. `app/core/security.py:L119` defines a second
`get_current_user`. `app/api/documents.py:L51`, `app/api/templates.py:L77` and
`app/api/users.py:L28` import the one defined here at L14.
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> User:
    ...
    then at L6 here. On a repaired import, `app/main.py:L125-L128` mounts every
    router with no prefix, so `GET /{document_id}` at
    `app/api/documents.py:L148` and `PUT /{document_id}` at `:L30` shadow
    `GET /me` and `PUT /me` at `app/api/users.py:L32` and `:L12`, and all five
    ...
    `app/core/security.py:L119` defines a second `get_current_user`. The
    missing-user branch at `app/core/security.py:L191` raises 401, where L25
    raises 404, and both send the detail `"User not found"`. The
    credential-failure detail differs too. L19 and L21 send
    `"Invalid authentication credentials"`, while `app/core/security.py:L184`
    and `app/core/security.py:L186` send `"Could not validate credentials"`.
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    `app/core/security.py:L50-L82`. L36 builds the payload
